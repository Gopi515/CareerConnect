...
$query = "SELECT a.id, a.profile, a.location, j.CTC, p.name, s.F_name, s.L_name, s.dept, s.email, s.start_year, s.end_year, s.pin, s.city, s.state, s.country 
    FROM job_applied a 
    ...
    WHERE a.status = ? AND CONCAT(a.id, ' ', a.profile, ' ', a.location, ' ', j.CTC, ' ', p.name, ' ', s.F_name, ' ', s.L_name, ' ', s.dept, ' ', s.email, ' ', s.start_year, ' ', s.end_year, ' ', s.pin, ' ', s.city, ' ', s.state, ' ', s.country, ' ') LIKE ? LIMIT ?, ?";
...
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM job_applied a 
    ...
    LEFT JOIN com_personal_details p ON a.com_id = p.com_id
    LEFT JOIN stu_personal_details s ON a.stu_id = s.stu_id 
    WHERE a.status = ? AND CONCAT(a.id, ' ', a.profile, ' ', a.location, ' ', j.CTC, ' ', p.name, ' ', s.F_name, ' ', s.L_name, ' ', s.dept, ' ', s.email, ' ', s.start_year, ' ', s.end_year, ' ', s.pin, ' ', s.city, ' ', s.state, ' ', s.country, ' ') LIKE ?";
...
<?php
    if(isset($_POST["Export"])){
        ...
        header('Content-Disposition: attachment; filename=Job_Shortlisted_Student.csv');  
        $output = fopen("php://output", "w");  
		
        fputcsv($output, array('ID', 'First Name', 'Last Name', 'Email', 'Start Year', 'End Year', 'ZIP Code', 'City', 'State', 'Country', 'Company Name', 'Job Topic', 'Work Location', 'CTC'));
        $status = "Company shortlisted your application";  
        $query = "SELECT a.id, s.F_name, s.L_name, s.email, s.start_year, s.end_year, s.pin, s.city, s.state, s.country, p.name, a.profile, a.location, j.CTC 
                FROM job_applied a 
                LEFT JOIN job j ON a.job_id = j.id
                LEFT JOIN com_personal_details p ON a.com_id = p.com_id
                LEFT JOIN stu_personal_details s ON a.stu_id = s.stu_id
...
<head>
    ...
    <title>Job Shortlisted Student</title>
        ...
        <div class="inner-whole-body">
            <table class="tablebody" border="1">
                <tr>
                    ...
                    <!-- job table -->
                    <th onclick="sortTable(12)" data-column="12">CTC<span class="sort-icon"></span></th>
                </tr>
