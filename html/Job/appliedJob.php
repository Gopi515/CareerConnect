...
<?php
   ...
   $query = "SELECT a.profile, a.location, a.status, d.apply_date, p.name, COUNT(a.id) AS id_count
        FROM job_applied a
        LEFT JOIN job_application_details d ON a.job_id = d.job_id
        LEFT JOIN com_personal_details p ON a.com_id = p.com_id
        WHERE a.stu_email = ? AND CONCAT(p.name, ' ', a.profile, ' ', a.location, ' ', a.status, ' ', d.apply_date) LIKE ?
        GROUP BY a.job_id
        LIMIT ?, ?";
    ...
    $totalRecordsQuery = "SELECT COUNT(*) AS total
        FROM job_applied a WHERE a.stu_email = ? AND CONCAT(a.profile, ' ', a.location, ' ', a.status) LIKE ?";
...
<body>
    ...
    <a href="../Job/job.php">
        <div class="regallclosebtn"><i class="fa-regular fa-circle-left" title="back to job"></i></div>
    </a>
        ...
        <div class="inner-whole-body">
            <table border="1">
                <tr>
                    ...
                    <th onclick="sortTable(3)" data-column="3">Applied on<span class="sort-icon"></span></th>
                    <th onclick="sortTable(4)" data-column="4">Numbar of Applicants<span class="sort-icon"></span></th>
                    <th onclick="sortTable(5)" data-column="5">Application Status<span class="sort-icon"></span></th>
                </tr>
